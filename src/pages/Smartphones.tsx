...
import { Smartphone as SmartphoneIcon, SlidersHorizontal } from 'lucide-react';
import { smartphones as allSmartphones, formatPrice, getLowestPrice } from '../data/products';
...
export default function Smartphones() {
  ...
  const brands = useMemo(() => [...new Set(allSmartphones.map(p => p.brand))].sort(), []);

  const filtered = useMemo(() => {
    let result = [...allSmartphones];
  ...
  const priceRange = useMemo(() => {
    const prices = allSmartphones.map(p => getLowestPrice(p));
      ...
      <div className="bg-gradient-to-r from-emerald-700 to-teal-600 text-white py-10">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-emerald-100">
            Comparez les prix de {allSmartphones.length} smartphones dans {formatPrice(0).split(' ')[1] ? '' : ''}6 boutiques marocaines
          </p>
          ...
          <div className="flex-1">
            ...
            <div className="lg:hidden mb-4">
              ...
              >
                ...
                Filtres {filtered.length !== allSmartphones.length && `(${filtered.length}/${allSmartphones.length})`}
              </button>
              {showFilters && (
                <div className="mt-4">
                  <FilterSidebar
            ...
            <div className="flex items-center justify-between mb-6">
              <p className="text-sm text-gray-500">
                <span className="font-semibold text-gray-900">{filtered.length}</span> smartphones trouvés
              </p>
              {/* Desktop sort */}
            ...
            {filtered.length === 0 ? (
              <div className="bg-white rounded-2xl p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Aucun smartphone trouvé</h3>
